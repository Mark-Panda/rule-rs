...
use rule_rs::resolve_placeholders_in_str;
...
impl WeatherNode {
    ...
    async fn get_weather(&self, city: &str) -> Result<WeatherInfo, Box<dyn std::error::Error>> {
        ...
        debug!("Requesting weather data for url: {}", url);
...
impl NodeHandler for WeatherNode {
    async fn handle<'a>(
        ...
        debug!("Processing weather request for oldcity: {}", city);
        let newcity = resolve_placeholders_in_str(city, &ctx, &msg);

        debug!("Processing weather request for city: {}", newcity);

        // 调用天气 API
        let weather = self.get_weather(&newcity).await.map_err(|e| {
        ...
        "nodes": [
            ...
            },
            {
                ...
                "config": {
                    ...
                    "city": "${00000000-0000-0000-0000-000000000000.input.msg.data.city}",
                    "language": "zh"
                },
                "layout": { "x": 1300, "y": 100 }
            },
